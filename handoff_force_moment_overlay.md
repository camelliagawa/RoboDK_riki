...
- [ ] **【最優先】空運転(空研)ベースラインの取得と差引** … 砥石を逃がして同じ kenma を1本記録し、
      `plot_force_log.py 研磨.csv --baseline 空運転.csv` で差し引く。**これで初めて真の接触力が読める**。
      重力補償の本命（Stream Motion 無しで実機姿勢が取れないため、空運転差引が実用解）。
