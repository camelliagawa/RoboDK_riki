...
### デスクトップ起動（ショートカット）
- `record_force.bat` … ダブルクリックで記録開始（`--no-robodk --log` を実行）。デスクトップにはこの .bat の
  ショートカットを置く（.bat 本体はリポジトリに置いたまま。`%~dp0` で自フォルダに cd）。PowerShell 経由で
  起動するので conda base（`(base)`）の python がそのまま使われる。
- `plot_force.bat` … ダブルクリックで最新 CSV のグラフを表示。
- `.gitattributes` で .bat は CRLF 取り出しに固定（cmd 互換）。

### 記録CSVのグラフ化（plot_force_log.py）
- `python plot_force_log.py` … 最新の `force_log_*.csv` を自動選択し、力[N]・モーメント[N·m]を2段の
  時系列グラフに描画（X/Y/Zは色覚安全なOkabe-Ito、|F|/|M|は太線）。同名 `.png` を保存し画面表示。
  最大|F|・最大|M|・平均|F|・サンプル数/レートを端末にも出力。
- 引数: `[csv]` 明示指定 / `--no-show`（保存のみ）/ `--contact 3.0`（|F|≥3Nの区間を加工区間として薄く塗る）。
- 依存: `matplotlib`（`pip install matplotlib`）。
