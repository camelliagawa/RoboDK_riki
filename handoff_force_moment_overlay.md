...
- `--baseline 空運転.csv` … **空運転CSVを時刻で差し引き、重力/姿勢オフセットを除去**して表示（真の接触力を見る本命）。
  ズレる場合は `--baseline-shift 秒` で開始点を合わせる。出力PNGは `*_baselined.png`。
...
- [ ] **【最優先】差引を信頼できる形にする** … 現状の課題2点:
      (1) 同じ非接触姿勢でも run 間でオフセットが違う（空運転6.0N vs 研磨8.3N＝約2.3N差）。tare/ドリフトが run 間で非再現。
      (2) 空運転270s と 研磨276s で尺が違い、時刻差引は後半ほどズレる。
      → 対策: **tare後ツールに触れず・砥石設定を変えず「空運転→研磨」を同一セッション連続記録**（理想は1記録に連続）。
      尺も揃える。その上で `plot_force_log.py 研磨.csv --baseline 空運転.csv`（必要なら `--baseline-shift`）で差引 → 真の接触力。
- [ ] **（任意）歯(パス)で自動整列する差引** … 尺が変わる場合、時刻でなく接触パターンの相互相関でズレ推定して整列する差引を追加できる。
