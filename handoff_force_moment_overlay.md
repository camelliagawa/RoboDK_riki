...
- **既定ブランチは `main`**（2026-07-17 に旧既定 `claude/riki-handover-review-g7dngo` から `main` へ切替。全成果は `main` に入っている）。
- **次セッションの作業ブランチ: 未確定** → 既定ブランチ `main` から**新しいブランチを切って**作業すること。
- 使い方:
  - 既定ブランチ `main` から新しいブランチを切って作業する。切ったら**この欄にブランチ名を必ず書き込む**。
  - 作業が済んだら `main` への PR を作成 → マージ。未マージの PR が残っている間は、その PR の head ブランチをそのまま継続してコミットする。
- ルール: **各セッションの終わりに、この「次セッションの作業ブランチ」欄を必ず更新する。**

### 最近の更新（2026-07-17 セッション）
