...
### 起動オプション（ファイルを編集せず切替）
`--demo` / `--port` / `--baud` / `--robot` / `--always-on` / `--detect busy|joints|both` / `--log` / `--log-path` / `--no-robodk` / `--no-open` / `--rate`

- `--no-open` … 記録終了(Ctrl+C)後に **CSVの保存フォルダをエクスプローラーで自動表示しない**。既定は自動で開く（保存したCSVを選択した状態でフォルダが開く）。
- `--rate` … サンプリング周波数[Hz]。既定は **記録のみ(`--no-robodk`)=50Hz** / RoboDK連携=10Hz。接近/退避のスパイクをより細かく捉えたいとき上げる。

### `plot_force_log.py` のオプション
- `--contact N` … |F|≥N を加工区間として薄く塗る。
- `--seg-thr N`（既定1.0）… 接触区間の判定しきい値。**区間ごとの平均/最大力・空中ベースラインを表で出力**（左右の食い込み差の確認に有効）。
...
- [x] **実研磨データの取得と検証**（2026-07-16, `force_log_20260716_124230.csv`）。主要知見:
      - tare良好（開始/終了の空中 |F|≈0.1N、ドリフト無し）。
      - **左右非対称が顕著**: 右(HaR)は約8Nで136秒連続接触（退避移動中も接触継続＝砥石に押し込まれっぱなし）、
        左(HaL)は各パス1〜2秒・約2Nの点接触。→ 右バベルが研磨の大半を過剰力で担い、左はほとんど削れていない。
      - **姿勢依存の重力オフセット**: HaL区間の空中で |F|≈1.3N（Fz≈1.0）出る。開始1回のtareでは姿勢変化分を消せない。
        左側の実接触力は見かけ2Nからこれを引いて実質0.5〜1N。
- [ ] **左右の接触力を揃えるプロセス調整**（要実機作業）… 刃の砥石中心に対する位置ズレ/反転軸を点検。
      右を弱め・左を強め、両側を中庸に。`HaR=CNT30` / `HaL=CNT60` も揃える。
- [ ] **重力補償（任意）** … 砥石を逃がした「空運転」を1本記録し、姿勢オフセットのベースラインとして差し引く運用を検討。
...
- `make_shortcuts.bat` … デスクトップに `record_force` / `plot_force` ショートカットを自動作成（最初に一度ダブルクリック）。
