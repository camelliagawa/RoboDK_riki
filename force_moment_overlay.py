...
USE_DEMO_SIGNAL = True   # True: ダミー正弦波でRoboDK描画テスト / False: read_wrench()の実センサ値
...
def read_wrench(t=0.0):
    ...
    USE_DEMO_SIGNAL=False のとき、ここに既存の DynPick 読み取り処理を書く。
    例:
        raw = dynpick.read()                 # 既存の読み取り関数
        return raw.fx, raw.fy, raw.fz, raw.mx, raw.my, raw.mz
    """
    if USE_DEMO_SIGNAL:
        return (8 * math.sin(t * 1.5), 4 * math.cos(t * 1.1), 6 * math.sin(t * 0.7),
                0.10 * math.sin(t * 1.3), 0.05 * math.cos(t * 0.9), 0.08 * math.sin(t))
    # ↓↓↓ 既存の DynPick 読み取り呼び出しに差し替える ↓↓↓
    raise NotImplementedError('read_wrench(): 既存の DynPick 読み取り処理を接続してください')
