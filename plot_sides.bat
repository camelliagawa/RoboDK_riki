...
rem ============================================================
powershell -NoExit -ExecutionPolicy Bypass -Command "Set-Location -LiteralPath '%~dp0'; if (Test-Path 'air.csv') { Write-Host 'Using air.csv baseline (accurate, order-independent).' -ForegroundColor Green; python plot_force_log.py --baseline air.csv --baseline-persides --sides } else { Write-Host 'No air.csv -> using --auto-zero (rough). Put your air run as air.csv for accuracy.' -ForegroundColor Yellow; python plot_force_log.py --auto-zero --sides }"
